...
describe('les clés de traduction', () => {
  ...
  it('et les clés composées restent comptées, pas oubliées', async () => {
    ...
    // Passé à 45 le 2026-09-05 : les dix centres d'intérêt, composés à trois
    // endroits — la chip qui les fait choisir, la fiche d'annuaire qui les
    // montre, et la section du profil qui les relit. Ce que cette garde y perd
    // est rendu ailleurs : `centres-d-interet.test.ts` résout les dix clés
    // dans les deux langues, et lit la liste dans l'énumération Python plutôt
    // que de la recopier.
    expect(composees).toBeLessThan(45);
  });
});
