...
import { etatDeLaFiche } from '../src/screens/terrain/etatDeLaFiche';
...
describe('l’état d’une fiche, et la conduite qu’il appelle', () => {
  it('préparée sans remise : la visite reste à faire', () => {
    expect(etatDeLaFiche(FICHE())).toBe('brouillon');
  });

  it('reprise : le salon existe pour les créatrices', () => {
    expect(etatDeLaFiche(REMISE('qr', { used_at: '2026-08-19T14:00:00Z' }))).toBe('activee');
  });

  it('et une reprise dont le lien a expiré depuis reste une reprise', () => {
    // **Le cas qui diverge de « regarde l'expiration d'abord ».** L'expiration
    // d'un lien déjà consommé ne dit rien : afficher « expirée » sur un salon
    // qui travaille depuis un mois enverrait quelqu'un le revisiter.
    expect(
      etatDeLaFiche(
        REMISE('qr', { used_at: '2026-08-19T14:00:00Z', expires_at: '2026-08-20T10:00:00Z' }),
        new Date('2026-09-30T00:00:00Z'),
      ),
    ).toBe('activee');
  });

  it('remise et non reprise : le lien vaut encore, ou il ne vaut plus', () => {
    const maintenant = new Date('2026-08-21T00:00:00Z');
    expect(etatDeLaFiche(REMISE('email'), maintenant)).toBe('remis');
    expect(
      etatDeLaFiche(REMISE('email', { expires_at: '2026-08-20T10:00:00Z' }), maintenant),
    ).toBe('expiree');
  });

  it('retirée avant reprise : ni activée, ni à relancer', () => {
    expect(etatDeLaFiche(REMISE('qr', { revoked_at: '2026-08-20T09:00:00Z' }))).toBe('revoquee');
  });
});
