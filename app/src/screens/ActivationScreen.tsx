...
import { useApi, type EtapeActivation } from '../api';
...
export function ActivationScreen({
  ...
  const requete = useRequete<EtapeActivation[]>(
    (signal) => api.etapesDActivation(businessId, signal),
    { estVide: (etapes) => etapes.length === 0, dependances: [businessId] },
  );
  ...
  return (
    <Ecran requete={requete} titre={t('commerce.activationTitre')} testID="ecran-activation">
      {(etapes) => {
