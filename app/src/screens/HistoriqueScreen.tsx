 ...
 * reconnaître qu'une page est pleine — donc qu'il y en a peut-être d'autres.
 */
const PAGE = 50;
...
export function HistoriqueScreen({
  ...
  const requete = useRequete<HistoriqueDuCreateur>(
    (signal) => api.mesReservations({ statuts, limite: PAGE }, signal),
   ...
   *
   * **Le même défaut de fond que « My posts », et le même remède.** L'écran
   * s'arrêtait à la page par défaut du serveur : un onglet « terminées » cesse
   * d'être un historique dès la cinquantième réservation, sans que rien ne le
   * dise. Ici le filtre par onglet évitait la page gâchée — les lignes rendues
   * appartiennent toutes à l'onglet lu — mais pas la troncature.
   *
   * Recharger la requête entière pour une page de plus ferait clignoter les
   * onglets et leurs compteurs alors que seul le bas s'allonge.
   */
  ...
  async function charger(curseur: string) {
    setEnCours(true);
    try {
      const page = await api.mesReservations({ statuts, limite: PAGE, avant: curseur });
    ...
    >
      {(vue) => (
        <View style={{ gap: 12 }}>
          ...
          {/* **Le curseur est la dernière ligne reçue.** Le serveur pagine sur
              `created_at`, la colonne de son tri, et refuse le décalage
              numérique — il sauterait des lignes dès qu'une réservation est
              prise pendant la lecture. Rien à proposer quand la dernière page
              n'était pas pleine : un bouton qui ne ramène jamais rien fait
              douter de la fin de liste. */}
          {pleine ? (
            <Pressable
