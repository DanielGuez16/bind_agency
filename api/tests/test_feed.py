...
from app.models import BusinessMember, CapacityRule, SocialAccount, TierOffer, User
...
class TestLeFiltreDeDisponibilite:
    """« Libre aujourd'hui » et « sept prochains jours ».

    **Un choix sur ce qu'on regarde, jamais sur ce qu'on propose d'élargir.**
    C'est ce qui décide de son emplacement : il s'applique au fil rendu, après
    que `categories` et `rayons` ont été comptés sur l'ensemble large. Les
    appliquer avant ferait dire « élargir à 30 km · 9 salons » puis n'en montrer
    que trois, parce que les six autres n'ont pas de place demain.
    """

    async def _salon_ouvert_un_seul_jour(self, session: AsyncSession, jours_apres: int):
        """Un salon dont la seule journée d'ouverture est dans N jours."""
        b = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await session.execute(sa.delete(CapacityRule).where(CapacityRule.business_id == b.id))
        cible = (datetime.now(UTC) + timedelta(days=jours_apres)).weekday()
        await capacity_service.create_rule(
            session,
            business_id=b.id,
            payload=CapacityRuleCreate(
                weekday=cible, start_time=time(8, 0), end_time=time(20, 0), concurrent_slots=3
            ),
        )
        return b

    async def test_aujourd_hui_ecarte_ce_qui_n_ouvre_que_plus_tard(
        self, session: AsyncSession
    ) -> None:
        maintenant = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, maintenant)
        plus_tard = await self._salon_ouvert_un_seul_jour(session, jours_apres=3)
        await offre(session, plus_tard)
        user, _ = await createur(session)

        sans_filtre = await fil(session, user)
        avec_filtre = await fil(
            session, user, disponible=service.FenetreDeDisponibilite.AUJOURD_HUI
        )

        assert {c.business_id for c in sans_filtre.commerces} == {maintenant.id, plus_tard.id}
        assert {c.business_id for c in avec_filtre.commerces} == {maintenant.id}

    async def _salon_ferme_pendant(self, session: AsyncSession, jours: int):
        """Un salon ouvert tous les jours, mais fermé les N prochains.

        **Une fermeture et non une règle hebdomadaire**, parce qu'une règle
        revient tous les sept jours : « libre dans dix jours mais pas dans
        sept » ne s'exprime pas autrement. Sans ce montage, rien ne distingue
        la fenêtre de sept jours de l'horizon complet, et le remplacer par
        trente ne casse aucun test.
        """
        b = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        premier = datetime.now(UTC).date()
        for decalage in range(jours):
            await capacity_service.create_exception(
                session,
                business_id=b.id,
                payload=CapacityExceptionCreate(date=premier + timedelta(days=decalage)),
            )
        return b

    async def test_sept_jours_les_garde_tous_les_deux(self, session: AsyncSession) -> None:
        """L'autre sens. Un filtre qui écarterait tout passerait le test
        ...
        vue = await fil(session, user, disponible=service.FenetreDeDisponibilite.SEPT_JOURS)

        assert {c.business_id for c in vue.commerces} == {maintenant.id, plus_tard.id}

    async def test_sept_jours_n_est_pas_l_horizon_complet(self, session: AsyncSession) -> None:
        """**Ce qui sépare la fenêtre de l'horizon.** Un salon fermé les huit
        prochains jours reste dans le fil sans filtre — il a des places dans
        l'horizon de réservation — et disparaît sous « sept prochains jours ».
        Sans ce cas, porter la fenêtre à trente jours ne casserait rien."""
        proche = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, proche)
        lointain = await self._salon_ferme_pendant(session, jours=8)
        await offre(session, lointain)
        user, _ = await createur(session)

        sans_filtre = await fil(session, user)
        sept = await fil(session, user, disponible=service.FenetreDeDisponibilite.SEPT_JOURS)

        assert {c.business_id for c in sans_filtre.commerces} == {proche.id, lointain.id}
        assert {c.business_id for c in sept.commerces} == {proche.id}

    async def test_les_comptes_ne_bougent_pas_avec_le_filtre(self, session: AsyncSession) -> None:
        """**Le cœur de la décision d'emplacement.** `categories` et `rayons`
        disent ce qu'un élargissement rapporterait : ils se calculent sur
        l'ensemble large, sans ce filtre. S'ils le suivaient, l'écran vide
        promettrait des salons qu'il ne montrerait pas."""
        maintenant = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, maintenant)
        plus_tard = await self._salon_ouvert_un_seul_jour(session, jours_apres=3)
        await offre(session, plus_tard)
        ...
        assert len(avec_filtre.commerces) < len(sans_filtre.commerces), "le filtre n'a rien filtré"
        assert {(c.categorie, c.commerces) for c in avec_filtre.categories} == {
            (c.categorie, c.commerces) for c in sans_filtre.categories
        }
        assert {(r.rayon_metres, r.commerces) for r in avec_filtre.rayons} == {
            (r.rayon_metres, r.commerces) for r in sans_filtre.rayons
        }

    async def test_un_item_sans_reservation_passe_toujours(self, session: AsyncSession) -> None:
        """On se présente quand on veut : « libre aujourd'hui » est vrai par
        construction. L'écarter reviendrait à cacher ce qui est le plus
        disponible du fil."""
        b = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await session.execute(sa.delete(CapacityRule).where(CapacityRule.business_id == b.id))
        await offre(session, b, duration_minutes=None, requires_booking=False)
        user, _ = await createur(session)

        vue = await fil(session, user, disponible=service.FenetreDeDisponibilite.AUJOURD_HUI)

        assert {c.business_id for c in vue.commerces} == {b.id}


class TestLaRecherche:
    """Chercher dans le fil, sur le nom du salon et celui de la prestation.

    **Dans la requête du fil, pas à côté.** Elle traverse donc le même tamis :
    paliers accessibles, offre active, item disponible, créneau restant. Un
    résultat qu'on ne peut pas réserver serait pire qu'aucun résultat.
    """

    async def test_elle_trouve_un_salon_par_son_nom(self, session: AsyncSession) -> None:
        cible = await commerce(
            session, name="Ocean Beauty Studio", longitude=ICI.longitude, latitude=ICI.latitude
        )
        await offre(session, cible)
        autre = await commerce(
            session, name="Wynwood Nails", longitude=ICI.longitude, latitude=ICI.latitude
        )
        await offre(session, autre)
        user, _ = await createur(session)

        vue = await fil(session, user, recherche="ocean")

        assert {c.business_id for c in vue.commerces} == {cible.id}

    async def test_elle_trouve_une_prestation_par_son_nom(self, session: AsyncSession) -> None:
        """Le nom de la prestation compte autant que celui du salon : on cherche
        « manucure », pas le salon qui la propose."""
        b = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, b, name="Manucure russe")
        autre = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, autre, name="Brushing")
        user, _ = await createur(session)

        vue = await fil(session, user, recherche="manucure")

        assert {c.business_id for c in vue.commerces} == {b.id}

    async def test_elle_trouve_sans_accent_ce_qui_en_porte(self, session: AsyncSession) -> None:
        """**Miami est bilingue, et c'est le cas qui justifie `unaccent`.**
        « Panadería » se tape sans accent sur un clavier anglais ; sans cette
        extension, la moitié du marché est introuvable, et le défaut se présente
        comme un fil vide."""
        b = await commerce(
            session, name="Panadería del Sol", longitude=ICI.longitude, latitude=ICI.latitude
        )
        await offre(session, b)
        user, _ = await createur(session)

        assert {
            c.business_id for c in (await fil(session, user, recherche="panaderia")).commerces
        } == {b.id}
        # Et dans l'autre sens : tapé avec l'accent, ça marche aussi.
        assert {
            c.business_id for c in (await fil(session, user, recherche="Panadería")).commerces
        } == {b.id}

    async def test_elle_ignore_la_casse(self, session: AsyncSession) -> None:
        """Personne ne tape ses majuscules dans un champ de recherche."""
        b = await commerce(
            session, name="Ocean Beauty Studio", longitude=ICI.longitude, latitude=ICI.latitude
        )
        await offre(session, b)
        ...
        vue = await fil(session, user, recherche="OCEAN beauty")

        assert {c.business_id for c in vue.commerces} == {b.id}

    async def test_un_joker_tape_par_l_utilisateur_ne_rend_pas_tout(
        self, session: AsyncSession
    ) -> None:
        """**L'autre sens, et il est facile à rater.** Un `%` cherche un pour
        cent ; laissé tel quel il devient un joker qui rend le catalogue entier,
        et l'écran donnerait tout à qui a cherché « 50% »."""
        b = await commerce(session, name="Salon A", longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, b)
        user, _ = await createur(session)

        assert (await fil(session, user, recherche="%")).commerces == ()
        # Et un terme qui ne correspond à rien ne rend rien non plus.
        assert (await fil(session, user, recherche="zzzz")).commerces == ()

    async def test_sans_terme_le_fil_ne_change_pas(self, session: AsyncSession) -> None:
        """Une chaîne vide ou faite d'espaces n'est pas une recherche : c'est le
        champ qu'on vient d'effacer, et le fil doit revenir entier."""
        b = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, b)
        user, _ = await createur(session)

        entier = await fil(session, user)
        for terme in (None, "", "   "):
            vue = await fil(session, user, recherche=terme)
            assert {c.business_id for c in vue.commerces} == {
                c.business_id for c in entier.commerces
            }, f"le terme {terme!r} a changé le fil"


class TestLesSuggestions:
    """Ce qu'on propose avant que la créatrice ait tapé quoi que ce soit.

    Trois exigences, et chacune a son test :

    1. **Le même tamis que le fil.** Une suggestion qu'on ne peut pas réserver
       est pire qu'aucune suggestion : elle envoie sur une impasse quelqu'un qui
       cherchait de l'aide.
    2. **« Populaire » est vrai ou se tait.** Le corps dit lequel des deux il
       rend, et l'écran change de mot — un salon proche annoncé comme populaire
       est un mensonge que personne ne peut vérifier.
    3. **Le quartier vient de la position**, jamais d'un paramètre.
    """

    async def _suggestions(self, session, user, **kwargs):
        return await service.suggestions_du_createur(
            session, creator_id=user.id, autour_de=ICI, **kwargs
        )

    async def test_elle_rend_les_deux_groupes(self, session: AsyncSession) -> None:
        b = await commerce(
            session,
            name="Ocean Beauty",
            longitude=ICI.longitude,
        ...
        vue = await self._suggestions(session, user)

        assert {p.nom for p in vue.prestations} == {"Soin visage", "Brushing"}
        assert [s.nom for s in vue.salons] == ["Ocean Beauty"]
        # Un salon compte ses prestations, il ne se répète pas une fois par item.
        assert vue.salons[0].prestations == 2

    async def test_elle_ne_propose_pas_ce_qu_on_ne_peut_pas_reserver(
        self, session: AsyncSession
    ) -> None:
        """**Le tamis, et c'est la première exigence.** Un palier fermé ne doit
        rien suggérer : proposer une prestation qu'on ne peut pas prendre est
        pire que ne rien proposer."""
        ouvert = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, ouvert, tier_id=STORY, name="Accessible")
        ferme = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, ferme, tier_id=REEL, name="Hors de portee")
        # Cinq mille abonnés : la story est ouverte, le reel non.
        user, _ = await createur(session, followers=5_000)

        vue = await self._suggestions(session, user)

        assert {p.nom for p in vue.prestations} == {"Accessible"}
        assert {s.business_id for s in vue.salons} == {ouvert.id}

    async def test_sans_historique_elle_annonce_la_proximite(self, session: AsyncSession) -> None:
        """**« Populaire » se tait quand il serait faux.** Un quartier neuf n'a
        aucune réservation servie : dire « populaire » y serait invérifiable."""
        b = await commerce(
            session,
            longitude=ICI.longitude,
            latitude=ICI.latitude,
        ...
        assert vue.origine is service.OrigineDesSuggestions.A_PROXIMITE
        assert vue.quartier is Neighborhood.WYNWOOD

    async def test_avec_un_historique_elle_annonce_le_populaire_et_le_classe(
        self, session: AsyncSession
    ) -> None:
        """L'autre sens, et il porte deux choses : le mot change, **et** l'ordre
        suit les réservations servies plutôt que la distance."""
        proche = await commerce(
            session,
            name="Le plus proche",
            longitude=ICI.longitude,
        ...
        await offre(session, proche)
        couru = await commerce(
            session,
            name="Le plus couru",
            longitude=ICI.longitude + 0.01,
            latitude=ICI.latitude,
            neighborhood=Neighborhood.WYNWOOD,
        )
        await offre(session, couru)
        await _consommer_une_reservation(session, couru)
        user, _ = await createur(session)

        vue = await self._suggestions(session, user)

        assert vue.origine is service.OrigineDesSuggestions.POPULAIRE
        assert [s.nom for s in vue.salons] == ["Le plus couru", "Le plus proche"], (
            "le plus servi passe devant le plus proche"
        )

    async def test_une_reservation_non_servie_ne_rend_personne_populaire(
        self, session: AsyncSession
    ) -> None:
        """**`consumed` et rien d'autre.** Une réservation prise puis
        abandonnée ne dit rien de ce qui marche : compter les `held` ferait
        remonter un salon que personne n'a jamais visité, et c'est le seul état
        ...
        await offre(session, proche)
        reserve = await commerce(
            session,
            name="Reserve mais pas servi",
            longitude=ICI.longitude + 0.01,
            latitude=ICI.latitude,
            neighborhood=Neighborhood.WYNWOOD,
        )
        await offre(session, reserve)
        await _reserver_sans_servir(session, reserve)
        user, _ = await createur(session)

        vue = await self._suggestions(session, user)

        assert vue.origine is service.OrigineDesSuggestions.A_PROXIMITE
        assert [s.nom for s in vue.salons] == ["Le plus proche", "Reserve mais pas servi"]

    async def test_le_quartier_vient_de_la_position(self, session: AsyncSession) -> None:
        """Il n'est pas demandé à l'appelant : c'est celui du salon ouvert le
        plus proche. Le lui demander reviendrait à accepter qu'il se trompe."""
        loin = await commerce(
            session,
            longitude=ICI.longitude + 0.02,
            latitude=ICI.latitude,
        ...
        await offre(session, pres)
        user, _ = await createur(session)

        vue = await self._suggestions(session, user)

        assert vue.quartier is Neighborhood.BRICKELL

    async def test_un_createur_sans_palier_ne_recoit_rien(self, session: AsyncSession) -> None:
        """Le vide est ici le bon résultat : sans palier ouvert, aucune
        suggestion n'est réservable, et c'est le fil qui explique pourquoi."""
        b = await commerce(session, longitude=ICI.longitude, latitude=ICI.latitude)
        await offre(session, b)
