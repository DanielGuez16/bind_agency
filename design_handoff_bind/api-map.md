...
| Inscription terrain | `GET`/`POST /admin/prospects` | suivi : `business_id, name, status, address, prepared_at, issued_at, expires_at, used_at, revoked_at, channel, opened_at, blocked_at, etat` | **les fiches assumées restent dans la liste** : sans elles, on ne sait pas combien de visites ont abouti. `etat` vaut `prepared`, `never_opened`, `opened_not_claimed`, `blocked_on_commitment` ou `claimed` — **trois états pour une fiche non activée**, parce qu'ils appellent trois gestes : revisiter, relancer, ou rien de tout cela — le produit coince. `channel` départage les deux méthodes de démarchage ; le taux d'activation par voie dit si un second passage rapporte plus qu'une relance |
