...
| Annuaire | `GET /business/{id}/creators` | `creator_id, first_name, last_name, city, bio, paliers_ouverts, audience_totale` et `comptes[] {platform, handle, followers, avatar_key, profil_url}` | **ce que l'abonnement achète** : sans abonnement, `subscription_required` — un refus, jamais une liste vide. `avatar_key` se sert par `GET /media/{cle}` ; **nulle veut dire « pas de photo », et le serveur n'en invente pas** — le repli appartient à l'écran, qui connaît la taille et le thème. `profil_url` est dérivée du pseudonyme, nulle sur une plateforme qu'on ne sait pas rattacher |
