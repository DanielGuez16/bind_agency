...
| Valeur | Emploi |
| --- | --- |
| `brand.500` `#FF5E00` | **Surface uniquement.** Ne s'écrit jamais. |
| `brand.700` `#A83E06` | **Texte uniquement.** Ne se pose jamais en fond plein. |
| `brand.600` `#DA5510` | État appuyé de tout ce qui est en 500. Porte l'encre à 4,68:1. |
...
| --- | --- | --- |
| `brand.500` sur blanc | 3,1:1 | **Interdit en texte**, quelle que soit la taille |
...
| blanc sur `brand.500` | 3,1:1 | **≥ 24 px seulement — le défaut du bloc** |
| encre sur `brand.600` | 4,7:1 | Toute taille — **l'état appuyé** |
| blanc sur `brand.600` | 4,0:1 | Jamais sous 24 px. Le libellé reste en encre à l'appui. |

> **Remesurée le 2026-08-17**, au passage de `brand.500` à `#FF5E00` (le vectoriel
> de la fondatrice ; `#F26B21` était une estimation lue sur une capture Instagram
> compressée). Les deux premières lignes bougent de trois centièmes et tiennent.
>
> **Lignes 1 et 4 sont la même paire, lue dans les deux sens** — le contraste est
> symétrique, `brand.500` sur blanc et blanc sur `brand.500` valent le même
> rapport. Elles ont porté deux chiffres différents parce qu'elles étaient tenues
> pour deux constats indépendants ; elles sont désormais dérivées d'un seul calcul.
> Les verdicts, eux, diffèrent légitimement : l'un juge un texte orange, l'autre un
> texte blanc, et le seuil applicable n'est pas le même selon le corps.
>
> **Les deux dernières lignes sont nouvelles, et c'est le défaut que la remesure a
> révélé.** La table s'arrêtait au 500 : l'état appuyé n'avait jamais été mesuré,
> alors qu'il porte le libellé du bouton principal à 15 px. `brand.600` valait
> `#D2500F`, où l'encre donnait 4,334:1 et le blanc 4,290:1 — **la valeur était
> assise sur le point de croisement où aucune des deux couleurs ne passe**. Le
> défaut n'était pas de 4 % avec l'encre, il était maximal dans les deux sens à la
> fois, et il précédait le changement d'orange.
>
> `brand.600` est éclairci à **`#DA5510`**. Le raisonnement est au §3 bis.
...
### 3 bis. L'état appuyé — pourquoi `brand.600` s'éclaircit

Avec un libellé en encre, `brand.600` ne peut aller que dans un sens : **plus
clair**. L'encre gagne du contraste sur un orange qui s'éclaircit et en perd sur
un orange qui s'assombrit. Assombrir aurait donc imposé de passer le libellé en
blanc pendant l'appui — techniquement supérieur sur le papier, puisque le blanc
y trouve une marge confortable et que l'appui gagnerait en franchise.

Ce n'est pas ce qui est retenu, pour une raison qui vient du gabarit de bureau.
Sur un écran tactile, l'appui est **caché sous le doigt** — l'argument « personne
ne lit un bouton pendant qu'il appuie dessus » y est juste. Depuis la v0.6, le
commerce et l'administration travaillent sur ordinateur, où le pointeur ne couvre
rien : l'état appuyé y est pleinement visible. C'est exactement là qu'un libellé
qui change de couleur au clic se remarque, et se remarque comme un défaut.
L'option qui gagnait en mesure perdait au seul endroit où l'état se regarde.

**`#DA5510`, et non `#D65310`.** Le déplacement minimal qui franchit le seuil
donnerait 4,520:1, soit quatre millièmes de marge. Un corps de 15 px lissé ne
rend pas le contraste que le calcul annonce ; une marge de quatre millièmes est
un seuil tenu sur le papier et perdu à l'écran. `#DA5510` tient **4,683:1**, avec
4 % de marge réelle.

**Ce que ça coûte :** l'écart d'appui passe de 1,400:1 à 1,296:1, soit un
assombrissement 20 % moins marqué. L'appui reste un assombrissement — le verbe du
système ne change pas — il est seulement moins profond. C'est le prix payé pour
que le libellé ne bouge pas, et il est payé sur la seule chose que l'appui ait à
faire : dire qu'il a été enregistré.

**Un effet de bord qui va dans le sens de la décision :** en éclaircissant le 600,
le blanc y perd ce qui lui restait — 4,290:1 sur l'ancienne valeur, **3,971:1** sur
la nouvelle. Il passait de justesse sous le seuil, il est maintenant franchement
dehors. La règle « le libellé reste en encre » n'a donc plus de cas limite à
arbitrer : elle est la seule lecture que la mesure autorise. Le blanc tient encore
le seuil des grands corps, à 3,0 — un titre de 24 px sur un fond appuyé resterait
légal, mais aucun n'existe.

**Ce qui n'est pas retenu :** ne rien changer et l'écrire comme exception. Une
exception mesurée reste défendable quand la valeur est simplement juste; elle ne
l'est pas quand la valeur est le pire orange possible pour porter du texte.
...
Deux gains sur l'ancien système, au-delà du fait de survivre à une palette d'une seule teinte :
